                    ...
                    <form action="<?php echo e(route('pencatatan.index')); ?>" method="GET" class="flex items-center gap-3">
                        ...
                        <div x-data="{ open: false }" class="relative">
                            <button type="button" @click="open = !open"
                                ...
                                <svg class="w-5 h-5 text-gray-400 transition-transform duration-200" :class="open ? 'rotate-180' : ''" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
                                </svg>
                            </button>

                            <div x-show="open" @click.away="open = false" x-transition
                                class="absolute z-10 mt-1 w-44 bg-white border border-gray-200 rounded-lg shadow-lg">
                                <ul class="max-h-56 overflow-y-auto text-sm text-gray-700">
                                    <li>
                        ...
                        <div class="relative">
                            <select name="status" onchange="this.form.submit()"
                                    class="w-44 pl-4 pr-8 py-1.5 text-sm border border-gray-200 rounded-lg focus:ring-green-500 focus:border-green-500 focus:ring-0">
                                <option value="">Semua Status</option>
                                <option value="sudah_dicatat" <?php if(request('status') == 'sudah_dicatat'): echo 'selected'; endif; ?>>Sudah Dicatat</option>
                                <option value="belum_dicatat" <?php if(request('status') == 'belum_dicatat'): echo 'selected'; endif; ?>>Belum Dicatat</option>
                            </select>
                        </div>
                        ...
                        <div class="relative">
                            <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari nama anggota..."
                                class="w-52 pl-4 pr-8 py-1.5 text-sm border border-gray-200 rounded-lg focus:ring-green-500 focus:border-green-500 focus:ring-0">
                        ...
                        class="px-3 py-1.5 text-sm text-gray-600 hover:text-gray-800 border border-gray-200 rounded-lg hover:bg-gray-50">
                            Reset
                        </a>
                    </form>
                </div>
            ...
            <div x-show="filterOpen" x-transition class="md:hidden mb-4">
                <form action="<?php echo e(route('pencatatan.index')); ?>" method="GET"
                    ...
                    <div class="relative">
                        <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari nama anggota..."
                            class="w-full pl-4 pr-8 py-1.5 text-sm border border-gray-200 rounded-lg focus:ring-green-500 focus:border-green-500 focus:ring-0">
                        <span class="absolute inset-y-0 right-0 px-3 flex items-center text-gray-500">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                            </svg>
                    ...
                    <div class="flex justify-end items-center gap-3 mt-2">
                        <a href="<?php echo e(route('pencatatan.index')); ?>"
                            class="px-4 py-1.5 text-sm text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50">
                            Reset
                        </a>
                        <button type="submit"
                ...
                <?php else: ?>
                    <?php $__empty_1 = true; $__currentLoopData = $anggotas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $anggota): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <div class="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-lg hover:border-green-500 transition-all duration-300 flex flex-col">
                                ...
                                <?php if($anggota->latestPencatatan): ?>
                                    ...
                                    <?php else: ?>
                                        ...
                                        <?php
                                            ...
                                            // Target: Jumlah ternak yang 'aktif' di master
                                            $jumlahTernakAktif = $anggota->ternaks_count ?? 0; 
                                            
                                            $jumlahDetailLengkap = 0;
                                            if ($pencatatan) {
                                                // PERBAIKAN: Hitung detail yang diisi DAN ternaknya masih aktif
                                                $jumlahDetailLengkap = $pencatatan->details->filter(function($detail) {
                                                    // 1. Pastikan detailnya sudah diisi (bukan placeholder)
                                                    if (empty($detail->kondisi_ternak)) {
                                                        return false;
                                                    }
                                                    // 2. Pastikan ternak masternya ada DAN statusnya 'aktif'
                                                    if ($detail->ternak && $detail->ternak->status_aktif === 'aktif') {
                                                        return true;
                                                    }
                                                    // Jika ternaknya 'nonaktif' (mati/terjual), jangan dihitung
                                                    return false;
                                                })->count();
                                            }
                                        ?>

                                        <?php if($jumlahDetailLengkap == 0 && $jumlahTernakAktif > 0): ?>
                                            
                                            <span class="text-xs font-bold px-2.5 py-1 rounded-full bg-yellow-100 text-yellow-800">
                                                Belum Dicatat
                                            </span>
                                        <?php elseif($jumlahDetailLengkap < $jumlahTernakAktif): ?>
                            ...
                            <div class="p-4 mt-auto border-t border-gray-200 bg-gray-50">
...
<?php endif; ?>
<?php /**PATH C:\MAHENDRA\Project\sipaten\resources\views/petugas/pencatatan.blade.php ENDPATH**/ ?>