...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.modal-base','data' => ['title' => 'Edit Pengguna','maxWidth' => 'md','scrollable' => false,'showTitle' => true]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? (array) $attributes->getIterator() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Edit Pengguna','maxWidth' => 'md','scrollable' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(false),'showTitle' => true]); ?>
    ...
    <p class="text-xs text-gray-500 mb-2">
        Edit informasi pengguna untuk memperbarui data akun
    </p>
    ...
    <form method="POST" action="<?php echo e(route('pengguna.update', $user)); ?>" class="space-y-3">
        <?php echo csrf_field(); ?>
        <?php echo method_field('PUT'); ?>

        
        <div>
            <label class="block text-xs font-medium text-gray-600">Nama User</label>
            <input type="text" name="name" value="<?php echo e(old('name', $user->name)); ?>"
                class="mt-1 block w-full rounded-md border-gray-200 text-gray-800 focus:ring-0 focus:ring-green-500 focus:border-green-500 text-xs"
                required>
        </div>
        ...
        <div class="grid grid-cols-4 gap-3">
            <div class="col-span-3">
                <label class="block text-xs font-medium text-gray-600">Email (Username)</label>
                <input type="email" name="email" value="<?php echo e(old('email', $user->email)); ?>"
                    class="mt-1 block w-full rounded-md text-gray-800 border-gray-200 focus:ring-0 focus:ring-green-500 focus:border-green-500 text-xs"
                    required>
            </div>

            
            <div x-data="{ open: false, selected: '<?php echo e(old('role', $user->role)); ?>' }" class="relative col-span-1">
                <label class="block text-xs font-medium text-gray-600">Role</label>
                <input type="hidden" name="role" x-model="selected">
                <button type="button" @click="open = !open"
                    class="mt-1 w-full px-2 py-2 border rounded-md text-left text-gray-800 bg-white border-gray-200 text-xs focus:ring-green-500 focus:border-green-500 capitalize">
        ...
        <div class="grid grid-cols-4 gap-3">
            ...
            <div class="col-span-3 relative" x-data="{ show: false }">
                <label class="block text-xs font-medium text-gray-600">Password (kosongkan jika tidak diubah)</label>
                <input 
                    :type="show ? 'text' : 'password'" 
                    name="password"
                    class="mt-1 block w-full rounded-md text-gray-800 border-gray-200 focus:ring-0 focus:ring-green-500 focus:border-green-500 text-xs pr-8"
                    placeholder="••••••••"
                >

                
                <span class="absolute inset-y-0 right-3 flex items-center cursor-pointer mt-5"
                    @click="show = !show">
                    
                    <svg x-show="!show" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                        viewBox="0 0 24 24" class="w-4 h-4 text-gray-600">
            ...
            <div x-data="{ open: false, selected: '<?php echo e(old('status', $user->status)); ?>' }" class="relative col-span-1">
                <label class="block text-xs font-medium text-gray-600">Status</label>
                <input type="hidden" name="status" x-model="selected">
                <button type="button" @click="open = !open"
                    class="mt-1 w-full px-2 py-2 border rounded-md text-gray-800 text-left bg-white border-gray-200 text-xs focus:ring-green-500 focus:border-green-500 capitalize">
                    <span x-text="selected"></span>
                    <svg class="w-4 h-4 float-right text-gray-500 transform transition-transform duration-200"
                        :class="open ? 'rotate-180' : ''" xmlns="http://www.w3.org/2000/svg" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor">
...
<?php endif; ?>
<?php $__env->stopPush(); ?>
<?php /**PATH C:\MAHENDRA\Project\sipaten\resources\views/components/pengguna/edit-modal.blade.php ENDPATH**/ ?>