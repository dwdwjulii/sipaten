<x-app-layout>
    <div class="py-6">
        <div class="mx-auto max-w-full px-4 sm:px-6 lg:px-8">
            ...
            <form action="{{ route('pencatatan.store') }}" method="POST" enctype="multipart/form-data" id="pencatatan-form">
                @csrf
                <input type="hidden" name="anggota_id" value="{{ $anggota->id }}">
                <input type="hidden" name="pencatatan_id" value="{{ $pencatatanBulanIni->id }}">

                {{-- Main Layout Container --}}
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {{-- Left Column: Form Data (2/3 width on desktop) --}}
                    <div class="lg:col-span-2 space-y-6">
                        {{-- Data Anggota Ternak (Read Only) --}}
                        <div class="section-card bg-white rounded-lg shadow-md p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                ...
                                <div>
                                    ...
                                    <div class="p-3 bg-gray-100 border border-gray-200 rounded-md text-gray-900">
                                        {{ $anggota->ternaks()->where('tipe_ternak', 'Induk')->where('status_aktif', 'aktif')->count() }} ekor
                                    </div>
                                </div>
                                <div>
                        ...
                        <div id="ternakContainer" class="space-y-4">
                            ...
                            @foreach ($groupedTernaks as $group)

                                {{-- ======================================================= --}}
                                {{-- Grup untuk Induk yang Masih Aktif & Anaknya  --}}
                                {{-- ======================================================= --}}
                                @if ($group['type'] === 'active_parent')
                                    ...
                                    <div class="bg-white shadow rounded-lg ternak-group mb-4">
                                        {{-- Header Card untuk Induk --}}
                                        <div class="p-4 border-b border-gray-200">
                                            <div class="flex justify-between items-center">
                                                <h3 class="text-lg font-semibold text-gray-900">
                                                    Data Induk #{{ $group['group_index'] }}
                                                </h3>
                                                <button type="button" onclick="addAnak('{{ $induk->id }}', {{ $group['group_index'] }})" 
                                        ...
                                        <div class="p-6">
                                            ...
                                            <div class="mb-6">
                                                ...
                                                <input type="hidden" name="ternaks[{{ $induk->id }}][ternak_id]" value="{{ $induk->id }}">
                                                
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    <div>
                                                        <label class="block text-xs font-medium text-gray-700">Tipe Ternak</label>
                                                        <select name="ternaks[{{ $induk->id }}][tipe_ternak]" 
                                                            ...
                                                            <option value="Induk" {{ $induk->tipe_ternak === 'Induk' ? 'selected' : '' }}>Induk</option>
                                                            <option value="Anak" {{ $induk->tipe_ternak === 'Anak' ? 'selected' : '' }}>Anak</option>
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label class="block text-xs font-medium text-gray-700">Jenis Kelamin <span class="text-red-500">*</span></label>
                                                        <select name="ternaks[{{ $induk->id }}][jenis_kelamin]" required 
                                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 text-xs">
                                                            <option value="">Pilih Kelamin</option>
                                                            <option value="Jantan" {{ $induk->jenis_kelamin === 'Jantan' ? 'selected' : '' }}>Jantan</option>
                                                            <option value="Betina" {{ $induk->jenis_kelamin === 'Betina' ? 'selected' : '' }}>Betina</option>
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label class="block text-xs font-medium text-gray-700">No Ear Tag <span class="text-red-500">*</span></label>
                                                        <select name="ternaks[{{ $induk->id }}][no_ear_tag]" required 
                                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 text-xs">
                                                            <option value="">Pilih Ear Tag</option>
                                                            <option value="Ada" {{ $induk->no_ear_tag === 'Ada' ? 'selected' : '' }}>Ada</option>
                                                            <option value="Tidak Ada" {{ $induk->no_ear_tag === 'Tidak Ada' ? 'selected' : '' }}>Tidak Ada</option>
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label class="block text-xs font-medium text-gray-700">Umur <span class="text-red-500">*</span></label>
                                                        <input type="text" name="ternaks[{{ $induk->id }}][umur_ternak]" required 
                                                            value="{{ old('ternaks.'.$induk->id.'.umur_ternak', $induk->umur_ternak) }}"
                                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm placeholder:text-red-500 focus:border-green-500 focus:ring-green-500 text-xs" 
                                                            placeholder="Contoh: 6 bulan">
                                                    </div>
                                                    <div>
                                                        <label class="block text-xs font-medium text-gray-700">Kondisi <span class="text-red-500">*</span></label>
                                                        <select name="ternaks[{{ $induk->id }}][kondisi_ternak]" required 
                                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 text-xs">
                                                            <option value="">Pilih Kondisi</option>
                                                            <option value="Sehat" {{ old('ternaks.'.$induk->id.'.kondisi_ternak', $induk->kondisi_ternak) === 'Sehat' ? 'selected' : '' }}>Sehat</option>
                                                            <option value="Sakit" {{ old('ternaks.'.$induk->id.'.kondisi_ternak', $induk->kondisi_ternak) === 'Sakit' ? 'selected' : '' }}>Sakit</option>
                                                            <option value="Mati" {{ old('ternaks.'.$induk->id.'.kondisi_ternak', $induk->kondisi_ternak) === 'Mati' ? 'selected' : '' }}>Mati</option>
                                                            <option value="Terjual" {{ old('ternaks.'.$induk->id.'.kondisi_ternak', $induk->kondisi_ternak) === 'Terjual' ? 'selected' : '' }}>Terjual</option>
                                                        </select>
                                                    </div>
                                                    <div>
                                                        <label class="block text-xs font-medium text-gray-700">Status Vaksin <span class="text-red-500">*</span></label>
                                                        <select name="ternaks[{{ $induk->id }}][status_vaksin]" required 
                                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 text-xs">
                                                            <option value="">Pilih Vaksin</option>
                                                            <option value="Sudah" {{ old('ternaks.'.$induk->id.'.status_vaksin', $induk->status_vaksin) === 'Sudah' ? 'selected' : '' }}>Sudah</option>
                                                            <option value="Belum" {{ old('ternaks.'.$induk->id.'.status_vaksin', $induk->status_vaksin) === 'Belum' ? 'selected' : '' }}>Belum</option>
                                                        </select>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div id="anakContainer-{{ $group['group_index'] }}" class="space-y-3">
                                                {{-- Render anak yang sudah ada --}}
                                                @foreach($anakCollection as $anak)
                                                <div class="anak-section" id="existing-anak-{{ $anak->id }}">
                                                    <div class="flex justify-between items-center mb-3">
                                                        <h4 class="text-sm font-semibold text-gray-800">Data Anak Ternak</h4>
                                                        <button type="button" onclick="removeAnak('existing-anak-{{ $anak->id }}')" 
                                                            class="text-red-500 hover:text-red-700 transition-colors">
                                                            <i class="fas fa-times"></i> Hapus
                                                        </button>
                                                    </div>
                                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                        <input type="hidden" name="ternaks[{{ $anak->id }}][ternak_id]" value="{{ $anak->id }}">
                                                        <div>
                                                            <label class="block text-xs font-medium text-gray-700">Tipe Ternak</label>
                                                            <select name="ternaks[{{ $anak->id }}][tipe_ternak]" 
                                                                class="tipe-ternak-select mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 text-xs" 
                                                                onchange="updateTernakSummary()">
                                                                <option value="Induk" {{ $anak->tipe_ternak === 'Induk' ? 'selected' : '' }}>Induk</option>
                                                                <option value="Anak" {{ $anak->tipe_ternak === 'Anak' ? 'selected' : '' }}>Anak</option>
                                                            </select>
                                                        </div>
                                                        <div>
                                                            <label class="block text-xs font-medium text-gray-700">Jenis Kelamin <span class="text-red-500">*</span></label>
                                                            <select name="ternaks[{{ $anak->id }}][jenis_kelamin]" required 
                                                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 text-xs">
                                                                <option value="">Pilih Kelamin</option>
                                                                <option value="Jantan" {{ $anak->jenis_kelamin === 'Jantan' ? 'selected' : '' }}>Jantan</option>
                                                                <option value="Betina" {{ $anak->jenis_kelamin === 'Betina' ? 'selected' : '' }}>Betina</option>
                                                            </select>
                                                        </div>
                                                        <div>
                                                            <label class="block text-xs font-medium text-gray-700">No Ear Tag <span class="text-red-500">*</span></label>
                                                            <select name="ternaks[{{ $anak->id }}][no_ear_tag]" required 
                                                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 text-xs">
                                                                <option value="">Pilih Ear Tag</option>
                                                                <option value="Ada" {{ $anak->no_ear_tag === 'Ada' ? 'selected' : '' }}>Ada</option>
                                                                <option value="Tidak Ada" {{ $anak->no_ear_tag === 'Tidak Ada' ? 'selected' : '' }}>Tidak Ada</option>
                                                            </select>
                                                        </div>
                                                        <div>
                                                            <label class="block text-xs font-medium text-gray-700">Umur <span class="text-red-500">*</span></label>
                                                            <input type="text" name="ternaks[{{ $anak->id }}][umur_ternak]" required 
                                                                value="{{ old('ternaks.'.$anak->id.'.umur_ternak', $anak->umur_ternak) }}"
                                                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm placeholder:text-red-500 focus:border-green-500 focus:ring-green-500 text-xs" 
                                                                placeholder="Contoh: 6 bulan">
                                                        </div>
                                                        <div>
                                                            <label class="block text-xs font-medium text-gray-700">Kondisi <span class="text-red-500">*</span></label>
                                                            <select name="ternaks[{{ $anak->id }}][kondisi_ternak]" required 
                                                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 text-xs">
                                                                <option value="">Pilih Kondisi</option>
                                                                <option value="Sehat" {{ old('ternaks.'.$anak->id.'.kondisi_ternak', $anak->kondisi_ternak) === 'Sehat' ? 'selected' : '' }}>Sehat</option>
                                                                <option value="Sakit" {{ old('ternaks.'.$anak->id.'.kondisi_ternak', $anak->kondisi_ternak) === 'Sakit' ? 'selected' : '' }}>Sakit</option>
                                                                <option value="Mati" {{ old('ternaks.'.$anak->id.'.kondisi_ternak', $anak->kondisi_ternak) === 'Mati' ? 'selected' : '' }}>Mati</option>
                                                                <option value="Terjual" {{ old('ternaks.'.$anak->id.'.kondisi_ternak', $anak->kondisi_ternak) === 'Terjual' ? 'selected' : '' }}>Terjual</option>
                                                            </select>
                                                        </div>
                                                        <div>
                                                            <label class="block text-xs font-medium text-gray-700">Status Vaksin <span class="text-red-500">*</span></label>
                                                            <select name="ternaks[{{ $anak->id }}][status_vaksin]" required 
                                                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 text-xs">
                                                                <option value="">Pilih Vaksin</option>
                                                                <option value="Sudah" {{ old('ternaks.'.$anak->id.'.status_vaksin', $anak->status_vaksin) === 'Sudah' ? 'selected' : '' }}>Sudah</option>
                                                                <option value="Belum" {{ old('ternaks.'.$anak->id.'.status_vaksin', $anak->status_vaksin) === 'Belum' ? 'selected' : '' }}>Belum</option>
                                                            </select>
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                @if ($group['type'] === 'orphan')
                                    <div class="bg-white shadow rounded-lg ternak-group mb-4">
                                        ...
                                        <div class="p-6">
                                            @foreach($group['anak'] as $anak)
                                            <div class="anak-section" id="existing-anak-{{ $anak->id }}" style="border-left-color: #f97316;">
                                                <div class="flex justify-between items-center mb-3">
                                                    <h4 class="text-sm font-semibold text-gray-800">Data Anak Ternak</h4>
                                                    <button type="button" onclick="removeAnak('existing-anak-{{ $anak->id }}')" 
                                                        class="text-red-500 hover:text-red-700 transition-colors">
                    ...
                    {{-- Right Column: Documentation (1/3 width on desktop, full width on mobile) --}}
                                ...
                                <div class="bg-blue-50 rounded-lg p-4 mb-4 border border-blue-200">
                                    ...
                                    <div class="space-y-2 text-sm">
                                        <div class="flex justify-between">
                                            ...
                                            <span class="font-medium text-blue-900" id="totalInduk">{{ $anggota->ternaks->count() }}</span>
                                        </div>
                                        <div class="flex justify-between">
                                        ...
                                        <div class="flex justify-between border-t border-blue-200 pt-2 mt-2">
                                            ...
                                            <span class="font-bold text-blue-900" id="totalKeseluruhan">{{ $anggota->ternaks->count() }}</span>
                                        </div>
                                    </div>
                                </div>
                ...
                Array.from(input.files).forEach(file => {
                    if (file.size > maxSize) {
                        Swal.fire({
                            ...
                            text: `File ${file.name} terlalu besar. Maksimal 2MB`,
                            icon: 'error',
