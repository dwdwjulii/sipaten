<th class="px-2 py-3 border border-gray-200 text-center">
    <div x-data="createDropdown('Status', 80)" x-init="() => { 
        ...
        <template x-teleport="body">
            <div x-show="open" @click.outside="close()" @keydown.escape.window="close()" x-transition x-cloak
                class="bg-white border border-gray-200 rounded-md shadow-md z-[2000] overflow-hidden min-w-[100px]"
                ...
                <a href="#" @click.prevent="select('Aktif')"
                    class="block px-3 py-2 text-xs text-gray-700 hover:bg-green-100 whitespace-nowrap">Aktif</a>
                <a href="#" @click.prevent="select('Non-Aktif')"
                    class="block px-3 py-2 text-xs text-gray-700 hover:bg-green-100 whitespace-nowrap">Non-Aktif</a>
            </div>
        </template>
    </div>
</th>