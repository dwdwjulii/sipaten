<x-app-layout pagePadding="sm:px-24">
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4 mb-6">

        {{-- Card Kelola Pengguna --}}
        <a href="{{ route('pengguna.index') }}" class="flex items-center p-3 bg-white rounded-md shadow-md border border-gray-100 
           hover:shadow-lg transition-shadow duration-300 ease-in-out">
            ...
            <div>
                <p class="text-3xl font-extrabold text-green-700">{{ $totalPengguna }}</p>
                <p class="text-sm font-medium text-gray-500">Kelola Pengguna</p>
            </div>
        </a>

        {{-- Card Kelola Anggota --}}
        <a href="{{ route('anggota.index') }}" class="flex items-center p-3 bg-white rounded-md shadow-md border border-gray-100 
           hover:shadow-lg transition-shadow duration-300 ease-in-out">
            <div class="p-2 mr-4 mx-2 text-white bg-green-700 rounded-md ">
                <svg xmlns="http://www.w3.org/2000/svg" class="size-12" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
                <p class="text-3xl font-extrabold text-green-700">{{ $totalAnggota }}</p>
                <p class="text-sm font-medium text-gray-500">Kelola Anggota</p>
            </div>
        </a>

        {{-- Card Kelola Catatan (Link) --}}
        <a href="{{ route('pencatatan.index') }}" class="flex items-center p-3 bg-white rounded-md shadow-md border border-gray-100 
           hover:shadow-lg transition-shadow duration-300 ease-in-out">
            <div class="p-2 mr-4 mx-2 text-white bg-green-700 rounded-md ">
                <svg xmlns="http://www.w3.org/2000/svg" class="size-12" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"
        ...
        {{-- Card Arsip Laporan (Link) --}}
        <a href="{{ route('arsip.index') }}" class="flex items-center p-3 bg-white rounded-md shadow-md border border-gray-100 
           hover:shadow-lg transition-shadow duration-300 ease-in-out">
            <div class="p-2 mr-4 mx-2 text-white bg-green-700 rounded-md ">
                <svg xmlns="http://www.w3.org/2000/svg" class="size-12" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div x-data="{open: false, selected: 'Semua Anggota', search: ''}" class="relative w-56">
                        ...
                        <div x-show="open" @click.away="open = false" x-cloak
                            ...
                            <ul class="max-h-60 overflow-y-auto scrollbar-hide text-sm">
                                ...
                                <li x-show="'{{ strtolower($anggota->nama) }}'.includes(search.toLowerCase())">
                                    <button type="button"
                                        @click="selected = '{{ $anggota->nama }}'; open = false; loadChartData('{{ $anggota->id }}')"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // =====================================================================
    const persebaranKandang = @json($persebaranKandang);
