...
class PencatatanController extends Controller
{
        ...
        if (auth()->user()->role === 'admin') {
            ...
            if ($timestampSiklusTerbaru) {
                if ($semuaSiklusTerbaruDiarsip) {
                    ...
                    // Cari arsip berdasarkan created_at
                    $arsipTerbaru = Arsip::orderBy('created_at', 'desc')->first();
                    
                    // Cek apakah arsip terbaru sesuai dengan bulan/tahun siklus terbaru
                    ...
                    if ($adaArsipUntukSiklusTerbaru) {
                        ...
                        if ($adaTernakAktifUntukSiklusBaru) {
                            ...
                            $statusArsip = true; // <-- Alert 4
                        } else {
                            $statusKeseluruhan = null; 
                            $statusArsip = false; 
                        }
                    } else {
                        // Arsip terbaru bukan untuk siklus ini
                        $statusKeseluruhan = null;
                        $statusArsip = false;
                    }
                    ...
                    // siklus terbaru sedang aktif                    
                    $pencatatanSiklusTerbaruAktif = Pencatatan::where('tanggal_catatan', $timestampSiklusTerbaru)
                        ->where('is_locked', false) 
                        ->with(['details', 'anggota' => function ($q) {
                    ...
                    foreach ($pencatatanSiklusTerbaruAktif as $p) {
                        if (!$p->anggota) continue;

                        // Hitung ternak aktif saat ini
                        $jumlahTernakAktifSekarang = $p->anggota->ternaks()
    ...
    public function create(Anggota $anggota)
    {
        ...
        //  Cek apakah siklus terbaru itu aktif (masih ada yg 'is_locked = false')
        ...
        // bagian grup anak yatim (induknya sudah mati/terjual atau tidak punya induk)
        $anaksYatim = $ternaksAktif->filter(function($ternak) {
            ...
            // Tidak punya induk sama sekali (data lama / manual entry)
            if (!$ternak->induk_id) {
                return true;
            }
            
            // Cek kondisi induk menggunakan relasi 'induk'
        ...
        // Jika ada anak yatim, grup terpisah
    ...
    public function store(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            //  placeholder berdasarkan ID yang dikirim dari form
            ...
            if ($request->hasFile('foto_dokumentasi')) {
                foreach ($request->file('foto_dokumentasi') as $file) {
                    // penyimpanan folder dokumentasi_ternak tanpa prefix 'public/'
    ...
    public function exportLaporanKeseluruhan(Request $request)
    {
        ...
        foreach ($anggotas as $anggota) {
            ...
            // (anak yang induknya tidak ada di pencatatan)
        ...
        try {
            ...
            foreach ($request->ternaks as $key => $ternakData) {
                ...
                if (isset($ternakData['detail_id']) && !empty($ternakData['detail_id'])) {
                    ...
                    // Jika ternak mati/terjual, set status nonaktif
                        $updateData['status_aktif'] = 'nonaktif';
                    } else {
                        // Jika ternak sakit/sehat, set status aktif
